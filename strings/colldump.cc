...
static const uchar FORCE_INPUT[0] = {};

int main(int argc, char **argv) {
  if (argc >= 3 && !strcmp(argv[1], "--test")) {
    char src[4096];
    char dst[4096*4];
    bool debug = argc >= 4 && !strcmp(argv[3], "--debug");
    ...
    size_t readin;
    if (debug) {
      memcpy(src, FORCE_INPUT, sizeof(FORCE_INPUT));
      readin = sizeof(FORCE_INPUT);
    } else {
      readin = fread(src, 1, 4096, stdin);
    }
